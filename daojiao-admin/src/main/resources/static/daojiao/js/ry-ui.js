    ...
    $.extend({
    	...
    	table: {
            ...
            init: function(options) {
                ...
                $.btTable = $('#' + options.id);
                $.table.initEvent();
                var table = $('#' + options.id).bootstrapTable({
        ...
        imageinput:{
            init : function (applyId, applyCaseId) {
				var fileUrlMap = new Array();
				var arr = $(".imageinput");
				for(var i = 0; i<arr.length; i++) {
				    var url = $(arr[i]).parents(".form-group").find(".hidden");
				    var documentIdInput = $(arr[i]).parents(".form-group").find(".documentsInput");
				    var documentId = documentIdInput.val();
				    var urlStr = url.text();
				    // 回显文件
				    var urlArr = [];
				    if (urlStr !== undefined && urlStr !== '') {
                        urlArr = urlStr.replace("[", "").replace("]","").split(",");
                    }
				    // 回显文件删除配置
				    var deleteArr = [];
				    for(var x = 0; x < urlArr.length; x++ ){
				        deleteArr.push({
                            url:ctx + 'recovery/delete/document',
                            key: urlArr[x],
                            extra: {
                                applyId: applyId,
                    ...
                    $(arr[i]).fileinput({
                        ...
                        if (data.response.code === 0) {
                            layer.msg('文件上传成功！', { icon: $.modal.icon(modal_status.SUCCESS), time: 3000, shift: 5 });
                            var a =this.parentElement;
                            var  documentsInput  = $(a).parents(".form-group").find(".documentsInput");
                            var inputvalue = documentsInput.val();
                            inputvalue += "," + data.response.fileName;
                            documentsInput.val(inputvalue);

                            fileUrlMap.push({key: previewId, value: "," + data.response.fileName})
                        } else {
                            layer.msg('文件上传失败！', { icon: $.modal.icon(modal_status.FAIL), time: 3000, shift: 5 });
                        }
                    }).on('filesuccessremove', function (event, data, previewId, index) {
                        for (var i = 0; i < fileUrlMap.length; i++) {
                            if (fileUrlMap[i].key === data) {
                                var url = fileUrlMap[i].value;
                                var a = this.parentElement;
                                var  documentsInput  = $(a).parents(".form-group").find(".documentsInput");
                                var inputvalue = documentsInput.val();
                                if (inputvalue.indexOf(url) != -1) {
                                    inputvalue = inputvalue.replace(new RegExp(url, "g"), "");
                                    documentsInput.val(inputvalue);
                                }
                            }
                        }
        ...
        operate: {
			...
			remove1: function(name,url,id) {
				$.modal.confirm("确定删除该条" + name + "信息吗？", function() {
					var url1 = $.common.isEmpty(id) ? url : url.replace("{id}", id);
					if($.table._option.type == table_type.bootstrapTreeTable) {
						$.operate.get(url1);
					} else {
						var data = { "ids": id };
						$.operate.submit(url1, "post", "json", data);
					}
				});
			},
            // 批量删除信息
			...
			add1: function(name,url) {
				$.modal.open("添加" + name, url);
			},
            // 添加信息，以tab页展现
            addTab: function (id) {
			...
			edit1: function(name,url,id) {
				if($.common.isEmpty(id) && $.table._option.type == table_type.bootstrapTreeTable) {
					...
					var url1 = url.replace("{id}", row[$.table._option.uniqueId]);
					$.modal.open("修改" + name, url1);
				} else {
            ...
            successTabCallback: function(result) {
                if (result.code == web_status.SUCCESS) {
    	            ...
    	            var $contentWindow = $('.daojiao_iframe[data-id="' + currentId + '"]', topWindow)[0].contentWindow;
    	            $.modal.close();
